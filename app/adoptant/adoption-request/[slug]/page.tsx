...
import SubmitAdoptionRequestButton from "@/components/adoptant/SubmitAdoptionRequestButton";
...
type RequiredField = { key: keyof IAdoptant; label: string };
...
export default async function AdoptionRequestPage({
  ...
  if (!adoptantId || userRole !== "adoptant") {
    redirect(`/login?redirect=/adoptant/adoption-request/${slug}`);
  }

  // Fetch data
  const [listing, adoptant, existingRequests] = await Promise.all([
    getCatListing(slug),
    getAdoptantById(adoptantId),
    getAdoptionRequestsByAdoptant(adoptantId),
  ]);

  if (!listing || !adoptant) redirect("/adoption-listings");

  // Vérifie doublon
  const alreadyRequested = existingRequests.some(
    (r) => r.adoptionListing?.documentId === slug
  );

  // Champs manquants
  const missingFields = REQUIRED_FIELDS.filter(
    (f) => !adoptant[f.key] && adoptant[f.key] !== false
  );
  const responsibilityMissing = !adoptant.acceptsResponsibility;
        ...
        <div className="flex flex-col gap-8 max-w-4xl mx-auto">
          ...
          {alreadyRequested && (
            <div className="rounded-2xl bg-yellow-50 border border-yellow-200 px-6 py-5 flex flex-col gap-2">
              ...
              <Link
                href="/adoptant/profile?tab=demandes"
              ...
              <div className="flex flex-col gap-5">
                ...
                <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 flex flex-col gap-4">
                  ...
                  <ul className="flex flex-col gap-2.5">
                    {REQUIRED_FIELDS.map((f) => {
                      const filled = !!adoptant[f.key] || adoptant[f.key] === false;
                      return (
                    ...
                    <li className="flex items-center gap-3 text-sm">
                      <span
                        className={`inline-flex items-center justify-center w-5 h-5 rounded-full text-xs font-bold flex-shrink-0 ${
                          adoptant.acceptsResponsibility
                            ? "bg-green-100 text-green-700"
                            : "bg-red-100 text-red-600"
                        }`}
                      >
                      ...
                      >
                        ...
                        {!adoptant.acceptsResponsibility && " — manquant"}
                      </span>
                    </li>
                  </ul>
                  ...
                  {!canSubmit && (
                    <div className="rounded-xl bg-orange-50 border border-orange-200 px-4 py-3 text-sm text-orange-800">
                      ...
                      <Link
                        href="/adoptant/profile?tab=profil"
                ...
                <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 flex flex-col gap-3">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm">
                    {[
                      { label: "Prénom", value: adoptant.firstName },
                      { label: "Nom", value: adoptant.name },
                      { label: "Email", value: adoptant.email },
                      { label: "Téléphone", value: adoptant.phone },
                      { label: "Adresse", value: adoptant.address },
                      { label: "Ville", value: adoptant.city ? `${adoptant.postalCode} ${adoptant.city}` : null },
                    ].map(({ label, value }) => (
                      <div key={label}>
