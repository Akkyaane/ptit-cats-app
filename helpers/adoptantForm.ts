...
export type AdoptantFormValues = {
  name: string;
  firstName: string;
  email: string;
  password: string;
...
export const adoptantDefaultValues: AdoptantFormValues = {
  name: "",
  firstName: "",
  email: "",
  password: "",
...
export function mapAdoptantToFormValues(
  adoptant?: Partial<IAdoptant> | null,
): AdoptantFormValues {
  if (!adoptant) return adoptantDefaultValues;

  return {
    ...adoptantDefaultValues,
    name: stringValue(adoptant.name),
    firstName: stringValue(adoptant.firstName),
    email: stringValue(adoptant.email),
    birthDate: stringValue(adoptant.birthDate),
    phone: stringValue(adoptant.phone),
    address: stringValue(adoptant.address),
    postalCode: stringValue(adoptant.postalCode),
    city: stringValue(adoptant.city),
    householdComposition: stringValue(adoptant.householdComposition),
    roommatesCount: stringValue(adoptant.roommatesCount),
    hasChildren: yesNoValue(adoptant.hasChildren),
    childrenCount: stringValue(adoptant.childrenCount),
    childrenAges: stringValue(adoptant.childrenAges),
    householdAgreement: yesNoValue(adoptant.householdAgreement),
    disagreementDetails: stringValue(adoptant.disagreementDetails),
    workStatus: stringValue(adoptant.workStatus),
    profession: stringValue(adoptant.profession),
    workingHours: stringValue(adoptant.workingHours),
    aloneTime: stringValue(adoptant.aloneTime),
    housingType: stringValue(adoptant.housingType),
    housingSurface: stringValue(adoptant.housingSurface),
    livingEnvironment: stringValue(adoptant.livingEnvironment),
    nearBusyRoad: stringValue(adoptant.nearBusyRoad),
    petCanGoOutside: stringValue(adoptant.petCanGoOutside),
    apartmentFloor: stringValue(adoptant.apartmentFloor),
    windowsSecured: stringValue(adoptant.windowsSecured),
    planToSecureWindows: stringValue(adoptant.planToSecureWindows),
    hasGarden: stringValue(adoptant.hasGarden),
    gardenSurface: stringValue(adoptant.gardenSurface),
    gardenFenced: stringValue(adoptant.gardenFenced),
    fenceHeight: stringValue(adoptant.fenceHeight),
    hasBalconyOrTerrace: stringValue(adoptant.hasBalconyOrTerrace),
    balconySurface: stringValue(adoptant.balconySurface),
    balconySecured: stringValue(adoptant.balconySecured),
    hasOtherAnimals: yesNoValue(adoptant.hasOtherAnimals),
    otherAnimalsDetails: stringValue(adoptant.otherAnimalsDetails),
    sterilizedAnimals: yesNoValue(adoptant.sterilizedAnimals),
    petsSince: stringValue(adoptant.petsSince),
    remarks: stringValue(adoptant.remarks),
    acceptsResponsibility: booleanValue(adoptant.acceptsResponsibility),
  };
}

export function buildAdoptantFormData(
  values: AdoptantFormValues,
): FormData {
...
export function buildAdoptantPayload(
  formData: FormData,
